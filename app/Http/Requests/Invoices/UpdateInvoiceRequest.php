...
class UpdateInvoiceRequest extends FormRequest
{
    ...
    protected function prepareForValidation(): void
    {
        ...
        if ($this->input('items')) {
            ...
            $currentInvoice = Invoice::find($this->input('id'));
            $currentStockMovements = $currentInvoice->stockMovements;
            ...
            foreach ($this->input('items', []) as $index => $item) {
                if ($item['product_id'] && $item['warehouse_id'] && $this->input('invoice_type') === 'sales') {
                    ...
                    $currentStockMovement = $currentStockMovements->where('product_id', $item['product_id'])->where('warehouse_id', $item['warehouse_id'])->first();
                    $previousUsedQuantity = $currentStockMovement ? $currentStockMovement->quantity : 0;
                    
                    $available_quantity = match($currentStockMovement->movement_type) {
                        'in' => $currentWarehouseStockLevelQuantity - $previousUsedQuantity,
                        'out' => $currentWarehouseStockLevelQuantity + $previousUsedQuantity,
